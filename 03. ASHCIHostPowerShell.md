Deploy nested Azure Stack HCI 22H2 nodes with PowerShell
==============
Overview
-----------

With your Hyper-V host up and running, along with the management infrastructure, it's now time to deploy the Azure Stack HCI 22H2 nodes into VMs on your Hyper-V host.  For this, you'll be using a simple PowerShell script to automate much of the creation experience.
...
- [Create your first nested Azure Stack HCI 22H2 node](#create-your-first-nested-azure-stack-hci-22h2-node)
...
Create your first nested Azure Stack HCI 22H2 node
-----------
There are 3 main steps to create the virtualized Azure Stack HCI 22H2 node on our Hyper-V host:
...
When Hyper-V is running inside a virtual machine, the virtual machine must be turned off to adjust its memory. 
...
Note that simply enabling nested virtualization will have no effect on dynamic memory or runtime memory resize. 
The incompatibility only occurs while Hyper-V is running in the VM.

**NOTE** If you have additional capacity, feel free to allocate higher levels of memory to your AZSHCINODE01 VM.

Once the VM is successfully created, you should connect the Azure Stack HCI 22H2 ISO file, downloaded earlier.
...
With the VM running, and the boot process initiated, you should be in a position to start the deployment of the Azure Stack HCI 22H2 OS.
...
With the installation complete, you'll be prompted to change the password before logging in.  
Enter a password and exit to command line. Once complete, you should be at the **command prompt** on the "Welcome to Azure Stack HCI" screen.  Minimize the VM Connect window.

#### Configure Azure Stack HCI 22H2 node networking using PowerShell Direct ####
...
$azsHCILocalCreds = Get-Credential -UserName "Administrator" -Message "Enter the password used when you deployed the Azure Stack HCI 22H2 OS"
...
### Enable the Hyper-V role on your Azure Stack HCI 22H2 Node ###
There is an **bug** when running Azure Stack HCI 22H2 within a nested virtualization configuration, specifically, when using Windows Admin Center to enable the Hyper-V role, within a running instance of Azure Stack HCI 22H2, inside a **Generation 2 Hyper-V VM**.
To workaround this, you can run the following PowerShell command **from the Hyper-V host** to fix this issue.
...
Invoke-Command -VMName $nodeName -Credential $domainCreds -ScriptBlock {
    # Enable the Hyper-V role within the Azure Stack HCI 22H2 OS
...
Invoke-Command -VMName $nodeName -Credential $domainCreds -ScriptBlock {
    # Enable the Hyper-V PowerShell within the Azure Stack HCI 22H2 OS
...
-----------
You have now created your first Azure Stack HCI 22H2 node, inside a VM, running nested on Hyper-V.  You need a minimum of 2 nodes for deployment of an Azure Stack HCI 22H2 cluster, so **repeat the creation process** to add at least one additional node, or more, depending on your Hyper-V host resources.  Use AZSHCINODE0x for your node names and increment your IP addresses by 1 for each node you add, so 192.168.0.5, 192.168.0.6 and so on. - you should only have to change the **nodeName** and **newIP** parameters and then rerun the PowerShell commands.
